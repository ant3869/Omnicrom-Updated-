...
namespace Omnicrom
{
    ...
    {
        public static async Task RunScript(string script)
        {
            ...
            try
            {
                Log($"Executing PowerShell [{name}] script ...");

                runtimewatch.Start();
                //await PowerShellScriptWork(script);
                var result = Task.Run(async () => await PowerShellScriptWork(script)).Result;
                //var task = PowerShellScriptWork(script);
                //task.Wait();

                StopandGetFinishedRunTime(name, runtimewatch, result);             
            }
            catch (Exception ex) { Log($"Error executing powershell script: Exception {ex.Message} Trace {ex.StackTrace}"); }
        }

        private static async Task<string> PowerShellScriptWork(string script)
        {
        ...
        public static void StopandGetFinishedRunTime(string name, Stopwatch runtimewatch, string output = null)
        {
            runtimewatch.Stop();
            var result = Converter.ConvertAgeString(runtimewatch.Elapsed);
            runtimewatch.Reset();

            if (output != null)
                Log($"Process OUTPUT: {output}");

            Log($"Process [{name}] has finished. {result}");
        }
    }
